...
def get_sensors():
    with psycopg2.connect(database=os.getenv('DB'), user=os.getenv('DB_USER'), password=os.getenv('DB_PWD')) as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cursor:
            cursor.execute("SELECT sensors_id FROM sensors")
...
def get_sensor_units(user_sensor):
    with psycopg2.connect(database=os.getenv('DB'), user=os.getenv('DB_USER'), password=os.getenv('DB_PWD')) as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cursor:
            cursor.execute("SELECT units AS sensor_unit FROM sensors WHERE sensors_id = %s", (user_sensor,))
...
def get_sensor_parameter(user_sensor):
    with psycopg2.connect(database=os.getenv('DB'), user=os.getenv('DB_USER'), password=os.getenv('DB_PWD')) as conn:
        with conn.cursor(cursor_factory=RealDictCursor) as cursor:
            cursor.execute("SELECT parameter AS sensor_parameter FROM sensors WHERE sensors_id = %s", (user_sensor,))
